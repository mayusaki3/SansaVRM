...
##### 3.2.3.1 Meta VRM 拡張方針（非人型・URDF対応）

Meta VRM は、従来の VRM（人型アバター）に限定せず、
ロボット、多脚体、昆虫型等を含む汎用構造モデルを扱えるよう拡張する。

方針:
- Meta VRM は VRM の上位概念として定義する
- VRM 0.x / 1.x は入力・出力フォーマットの一種として扱う
- URDF 等の構造定義フォーマットも同一内部モデルへ統合可能とする
...
マッピング方針:
- 人型に対応可能な構造は Humanoid Mapping として扱う
- 非人型構造は Extended Mapping として保持する
- 非可逆変換は診断情報として通知する
