...
- `module_id` は model 内で一意でなければならない
- `slot_id` は module 内で一意でなければならない
- `state_id` は model 内で一意でなければならない
...
`SansaVRM_slots.current_connections` は各 Slot から見た局所参照である。

connections は有向グラフとして扱う。
from_slot_id から to_slot_id への接続は片方向として定義する。
必要に応じて双方向接続は2つの connection で表現する。
